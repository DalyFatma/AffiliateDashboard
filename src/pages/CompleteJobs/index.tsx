...
const CompleteJobs = () => {
  ...
  const result = AllQuotes.filter(
    (bookings) => bookings?.affiliate_id?._id! === user?._id! && bookings?.progress === "Completed"
  );
  console.log(result)
    ...
    },
    {
      ...
      selector: (row: any) =>
        row!.id_affiliate_driver! === undefined ? (
          ...
          <span>
            {row!.id_affiliate_driver?.firstname!} {row!.id_affiliate_driver?.surname!}
          </span>
        ),
      sortable: true,
    },
  ...
  return (
    <React.Fragment>
       {user.progress !== "100" ? <CheckProgress /> :  
