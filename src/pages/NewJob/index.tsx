...
const Bookings = () => {
  ...
  const result = AllQuotes.filter(
    (bookings) =>
      (bookings?.white_list?.includes(user?._id!) &&
        bookings.progress === "Booked") ||
      bookings.progress === "Refused"
  );
  ...
  function tog_AcceptJob() {
    setmodal_modal_AcceptJob(!modal_AcceptJob);
  }
  ...
  const handleAccept = () => {
    if (!selectedRow || !selectedRow._id) {
      console.error("Error: No quote selected.");
      return;
    }

    const affiliateId = user?._id!;
    const quoteId = selectedRow._id;

    console.log("Affiliate ID:", affiliateId);
    console.log("Quote ID:", quoteId);

    if (!affiliateId || !quoteId) {
      console.error("Error: Affiliate ID or Quote is undefined or null.");
      return;
    }
    ...
    const quote: Quote = {
      ...
      estimated_start_time: "",
      estimated_return_start_time: "",
      destination_point: { placeName: "", coordinates: { lat: 0, lon: 0 } },
      start_point: { placeName: "", coordinates: { lat: 0, lon: 0 } },
      vehicle_type: "",
      id_visitor: "",
      notes: "",
      createdAt: new Date(),
  ...
  const handleRefuse = () => {
    ...
    if (!affiliateId || !quoteId) {
      console.error("Error: Affiliate ID or Quote ID is undefined or null.");
      return;
    }
  ...
  //   console.log("Refuse function called");
  ...
  const [notesAcceptJob, setNotesAcceptJob] = useState("");
  const [sendButtonEnabled, setSendButtonEnabled] = useState(false);

  const handlePriceChange = (e: any) => {
    const newPrice = e.target.value;
    setPrice(newPrice);
    updateSendButtonStatus(newPrice, notesAcceptJob);
  };

  const handleProposedPriceChange = (e: any) => {
    const newPrice = e.target.value;
  ...
  const handleProposedPriceBlur = () => {
    const formattedPrice = formatPrice(price);
    setProposedPrice(formattedPrice);
  };
  ...
  const handleNotesChange = (e: any) => {
    ...
    setNotesAcceptJob(newNotes);
    updateSendButtonStatus(price, newNotes);
  };

  const updateSendButtonStatus = (newPrice: any, newNotes: any) => {
    if (newPrice.trim() !== "" || newNotes.trim() !== "") {
  ...
  // Function to handle input change and update progress
                    ...
                    <Row>
                      <Col lg={12} className="mb-2">
                        ...
                        <Form.Control
                          ...
                          id="proposed_price"
                          placeholder="£ 00.00"
                          onBlur={handleProposedPriceBlur}
                  ...
                  <Col lg={12}>
                    <div className="hstack gap-2 justify-content-end">
                      ...
                      <Button
                        ...
                        type="button"
                        onClick={() => {
                          isChecked ? handleAccept() : handleRefuse();
